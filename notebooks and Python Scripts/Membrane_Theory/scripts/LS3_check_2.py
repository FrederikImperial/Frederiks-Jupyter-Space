...
# %%
#this cell is only needed for multivariate optimization!!

import sys

# Argumente vom Batch-Skript lesen
arguments = sys.argv[1:]

# Array zum Speichern der Werte erstellen
t_strake_values = []

# Die Werte ausgeben und in das Array speichern
for i, arg in enumerate(arguments, start=102):
    print(f"Wert t_strake{i}: {arg}")
    t_strake_values.append(arg)

# Überprüfen des Arrays
print("t_strake_values Array:", t_strake_values)
...
# %%
def Utilization_for_Strake(Strake_ID_input, thickness_input):

    print('Strake_ID: ', Strake_ID_input)
    print('input thickness: ', thickness_input)
     #Create Subset of dataframe to select strake
    ...
    # Berechnung der Differenz zwischen dem größten und kleinsten Wert in der Spalte 'z' um L_strake zu erhalten
    L_strake = subset_df['z'].max() - subset_df['z'].min() + 1.0
    ...
    # Given parameters
    f_yk = 355.0  # in N/mm^2

    #plastic range factor
    beta_x = 1.0 - 0.75 / (1 + 1.1 * delta_0_over_t)

    #interaction exponent
    eta_x0 = 1.35 - 0.10 * delta_0_over_t
    eta_xp = 1.0 / (0.45 + 0.72 * delta_0_over_t)
    ...
    Utilization_output = sigma_x_Rd_t -(-min_N_z)
    ...
    return Utilization_output


# %%
# Liste zum Speichern der Dicken erstellen
thickness_values = []

# Dicken aus der Datei einlesen
with open("../data/input/Strake_Thicknesses.txt", "r") as file:
    # Die Dicken als Zeichenkette einlesen und in eine Liste von Fließkommazahlen umwandeln
    thickness_list = file.readline().strip().rstrip(",").split(",")
    # Jede Zeichenkette in eine Gleitkommazahl umwandeln
    thickness_values = [float(thickness) for thickness in thickness_list]

# Liste zum Speichern der Utilization-Werte erstellen
utilization_values = []

# Schleife durchführen, um Utilization für jeden Strake zu berechnen und in die Liste einzufügen
for i, strake_id in enumerate(range(102, 116)):
    # Dicke für den aktuellen Strake aus der Liste abrufen
    thickness_input = thickness_values[i]

    # Utilization für den aktuellen Strake berechnen
    utilization = Utilization_for_Strake(strake_id, thickness_input)

    # Utilization-Wert zur Liste hinzufügen
    utilization_values.append(utilization)

# Liste in eine Textdatei schreiben
with open("../data/output/output_Utilization_array.txt", "w") as file:
    for utilization in utilization_values:
        file.write(f"{utilization},")
...
try:
    with open(file_path, 'w') as file:
        file.write(f"Utilization = {utilization}\n")
